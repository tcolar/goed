...
// GET /v1/view/<viewid>/line_count returns the number of lines in the current view
func (a *Api) LineCount(w http.ResponseWriter, r *http.Request) {
	v, err := a.view(r)
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
	fmt.Fprintf(w, "%d", v.LineCount())
}
